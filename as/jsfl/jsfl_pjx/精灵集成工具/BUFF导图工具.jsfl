...
function exportPngWithConfigInterval(intervalArr, id) {
	...
	//创键Buff原件
	var picItemName = "dev/Buffer" + id;
	lib.addNewItem("movie clip", picItemName);
	lib.selectItem(picItemName, true);
	lib.setItemProperty('linkageExportForAS', true);
	lib.setItemProperty('linkageExportInFirstFrame', true);
	lib.setItemProperty('linkageClassName', 'mmo.battle.buffer.Buffer' + id);
	...
	for each(var item in lib.items) {
		if(item.itemType == "bitmap") {
			...
			if(frameIndex >= 0) {
				lib.setItemProperty('linkageExportForAS', true);
				lib.setItemProperty('linkageExportInFirstFrame', true);
				var exportName = 'buffer_' + id + '_' + itemName.substring(frameIndex, itemName.indexOf("."));
  ...
  finalDoc = fl.openDocument(saveURL + "/buffer" + id + ".fla");
  fl.setActiveWindow(finalDoc);
  ...
  fl.saveDocument(finalDoc2, saveURLNew + "/buffer" + id + ".fla");
  finalDoc2.publish();
  fl.closeDocument(finalDoc, false);
