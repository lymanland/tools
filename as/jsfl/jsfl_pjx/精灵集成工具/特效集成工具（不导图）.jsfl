...
function handleSingleFile(file) {
  ...
  //处理特效元件
	...
	doc.library.setItemProperty('linkageClassName', 'mmo.effect.Effect' + id);
  	doc.library.moveToFolder("dev", item.name);
  
  	//处理特效名元件
  	dealEffectName(doc, id);
...
function dealEffectName(doc, id) {
  	var items = doc.library.items;
 	var itemName = "";
	for(var i = 0; i < items.length; i++) {
		var item = items[i];
	...
	if(itemName != "") {
		doc.library.selectItem(itemName, true);
		doc.library.setItemProperty('linkageExportForAS', true);
		doc.library.setItemProperty('linkageExportInFirstFrame', true);
		doc.library.setItemProperty('linkageClassName', 'mmo.effect.EffectName' + id);
		doc.library.moveToFolder("dev", itemName, true);
	}
}
